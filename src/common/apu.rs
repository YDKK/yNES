...
impl Channel for Pulse {
  ...
  fn clock(&mut self) -> u8 {
    ...
    let sequencer = if self.current_time == 0 {
      self.current_time = self.timer;
      let result = Pulse::OUTPUT[self.duty as usize][self.current_sequencer_position as usize];
    ...
    if sequencer == false || length == 0 || self.current_time < 8 {
      0
    } else {
      envelope
    }
...
impl FrameCounter {
  fn clock(&mut self, pulse1: &mut Pulse, pulse2: &mut Pulse) -> bool {
    match self.count {
      3728 => {
        //エンベローブ, 三角波線形カウンタ
  ...
  fn set(&mut self, mode: bool, interrupt_inhibit: bool) {
    self.mode = mode;
...
impl Apu {
  pub fn new() -> Self {
    Apu {
      ...
      frame_counter: FrameCounter { mode: false, interrupt_inhibit: false, count: 0 },
      mixer: Mixer::new(),
      clock_count: 0,
    }
  }
  pub fn clock(&mut self) -> (f32, bool) {
    let irq = self.frame_counter.clock(&mut self.pulse1, &mut self.pulse2);

    let mut pulse1: u8 = 0;
    let mut pulse2: u8 = 0;
    if self.clock_count % 2 == 0 {
      pulse1 = self.pulse1.clock();
      pulse2 = self.pulse2.clock();
    }

    self.clock_count = self.clock_count.wrapping_add(1);
    (self.mixer.mix(pulse1, pulse2), irq)
  }
  pub fn write(&mut self, addr: u8, value: u8) {
    ...
    match addr {
      0x00 | 0x04 => {
        target.as_mut().unwrap().set_duty(value & 0b11000000 >> 6);
        target
          .as_mut()
          .unwrap()
        ...
        target.as_mut().unwrap().set_volume(value & 0x0F);
      }
      0x01 | 0x05 => {
        //TODO: Pulse Sweep
      }
      0x02 | 0x06 /* | 0x0A //TODO: Triangle*/ => {
        //Timer low
        target.as_mut().unwrap().set_timer_low(value);
      }
      0x03 | 0x07 /*| 0x0B | 0x0F*/ => {
        target.as_mut().unwrap().set_length(value & 0xF8 >> 3);
        if addr != 0x0F {//Noiseにはタイマーがない
          target.as_mut().unwrap().set_timer_high(value & 0b111);
        }
        target.as_mut().unwrap().envelope.start = true;
      }
      0x15 => {
        //TODO: DMC
        //TODO: Noise
        //TODO: Triangle
        self.pulse1.set_enable(value & 0b10 == 0b10);
        self.pulse2.set_enable(value & 0b1 == 0b1);
      }
      0x17 => {
        self.frame_counter.set(value & 0x80 == 0x80, value & 0x40 == 0x40);
      }
      _ =>{}//TODO
    }
  }
